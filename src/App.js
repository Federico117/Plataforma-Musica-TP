...
function Principal() {
  const [listplay, setListPlay] = useState([
    {titulo: 'Clasicos', descripcion: 'Coleccion de canciones clasicas', imagenUrl: albums[10].imagen},
    {titulo: 'Alegres', descripcion: 'Colección de canciones alegres', imagenUrl: albums[1].imagen},
    {titulo: 'Varios', descripcion: 'Playlist de temas variados', imagenUrl: albums[6].imagen}
  ]);
  ...
  return (
    <div style={{ display: 'flex', width: '100vw' }}>
      <SideBar alternar={alternar} setAlternar={setAlternar}>
        ...
        <PlayList tituloPlay={titulo} descripPlay={descripcion} imagenPlay={imagenUrl}/>)
        )}
      </SideBar>
      {alternar ? <ContenedorPrincipal /> : <Formulario lista={listplay} setList={setListPlay}/>}
    </div>
  )
}
function SideBar({alternar, setAlternar, children}){
  ...
  return (
    <div style={{ width: '300px'}}>
        <button type="button" className="boton-del-sitio" onClick={handleClick}>Nueva Playlist</button>
        {children}
      </div>
  )
}
...
function Formulario({lista, setList}) {
  ...
  function handleSubmit(event){
    ...
    copiaLista.push({titulo: title,descripcion: description, imagenUrl: imageUrlText});
  ...
  return (
    <div className="contenedor-form">
      ...
      <form className="formulario" onSubmit={handleSubmit}>
        ...
        <input type="text" id="title" className="input-del-sitio" value={title} onChange={handleChangeTit}/><br />

        <label htmlFor="description">Descripción:</label><br />
        <input type="text" id="description" className="input-del-sitio" value={description} onChange={handleChangeDesc}/><br />

        <label htmlFor="urlImagen">Imagen(url):</label><br />
        <input type="text" id="urlImagen" className="input-del-sitio" value={imageUrlText} onChange={handleChangeTextImg}/><br />

        <button disabled={title.length < 1 | description.length < 1 | imageUrlText.length < 1} type="submit" className="boton-del-sitio">Agregar Playlist</button>
      </form>
    </div>
